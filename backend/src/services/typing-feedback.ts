...
const minTestsRequired = Number.parseInt(
  process.env["TYPING_FEEDBACK_MIN_TESTS"] ?? "",
  10,
);
const MIN_TESTS =
  Number.isFinite(minTestsRequired) && minTestsRequired > 0
    ? minTestsRequired
    : DEFAULT_MIN_TESTS;
...
const feedbackCache = new LRUCache<string, TypingFeedback>({
  max: 500,
  ttl: CACHE_TTL_MS,
});
...
function dbResultToSessionInput(result: DBResult): TypingSessionInput {
  return {
    wpm: result.wpm,
    acc: result.acc,
    ...
    keySpacingStats: result.keySpacingStats,
    keyDurationStats: result.keyDurationStats,
  };
}
...
function localCacheKey(sessions: TypingSessionInput[]): string {
  return `local:${JSON.stringify(sessions.slice(0, 5).map((s) => s.wpm + s.acc))}:${sessions.length}`;
}

export async function getTypingFeedbackFromSessions(
  sessions: TypingSessionInput[],
  options?: { refresh?: boolean; source?: "local" | "account" },
  ...
  const cacheKey = localCacheKey(recentSessions);
...
async function enhanceWithLlm(
  ...
  return {
    ...
    poweredByAi: true,
    poweredByCursor: usesCursorAgent() ? true : undefined,
  };
}

export async function getTypingFeedbackForUser(
  uid: string,
  options?: { refresh?: boolean },
): Promise<TypingFeedback> {
  const cached = feedbackCache.get(uid);
  if (cached !== undefined && options?.refresh !== true) return cached;

  const results = await ResultDAL.getResultsForTypingFeedback(uid, MAX_RESULTS);
  ...
  const sessionInputs = results.map(dbResultToSessionInput);
  const feedback = await getTypingFeedbackFromSessions(sessionInputs, {
    refresh: options?.refresh,
    source: "account",
  });

  feedbackCache.set(uid, feedback);

  return feedback;
}

export function getTypingFeedbackMinTests(): number {
  return MIN_TESTS;
}
