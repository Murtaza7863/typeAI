...
export async function getLocalTypingFeedback(
  req: MonkeyRequest<undefined, GetLocalTypingFeedbackRequest>,
): Promise<GetLocalTypingFeedbackResponse> {
  const feedback = await getTypingFeedbackFromSessions(req.body.sessions, {
    source: "local",
  });

  return new MonkeyResponse("Typing feedback generated", feedback);
}
