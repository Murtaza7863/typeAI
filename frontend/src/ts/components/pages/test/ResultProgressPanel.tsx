...
export function ResultProgressPanel(): JSXElement {
  ...
  createEffectOn(getResultVisible, async (visible) => {
    if (!visible) {
      setSnapshot(null);
      return;
    }

    const context = getTestProgressContext();
  ...
  return (
    <Show when={getResultVisible() && snapshot()}>
      {(data) => {
        const progress = data();
              ...
              <Show when={progress.vsLast10Avg}>
                {(avg) => (
                  <Section title="vs your average">
                    <p>
                      {avg().wpmDelta >= 0 ? "Above" : "Below"} 10-test average
                      by{" "}
