...
The hosted site calls **`POST /api/typing-feedback`** on the same domain (Vercel serverless function). It does **not** use `api.monkeytype.com/dev/typingFeedback`.
...
| `BACKEND_URL` | `https://api.monkeytype.com` |
...
3. Network tab should show `POST /api/typing-feedback` (not `api.monkeytype.com/dev/...`).
